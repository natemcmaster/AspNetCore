config({
    resolvers: [
        {
            kind: "MsBuild",
            moduleName: "native-windows",
            root: d`.`,

            globalProperties: Map.empty<string, string>()
                .add("Configuration", Environment.hasVariable('Configuration')
                    ? Environment.getStringValue('Configuration')
                    : 'Debug'),
            ...
            fileNameEntryPoints: [r`src/native-windows.builds`],

            // If the location of MSBuild is set beforehand, honor it. Otherwise leave it to the default behavior.
            msBuildSearchLocations: [
                d`C:/Program Files (x86)/Microsoft Visual Studio/2019/Enterprise/MSbuild/Current/Bin/`
            ],


            // might be needed for C#
            ...
            // Set to true to enable binlog output
